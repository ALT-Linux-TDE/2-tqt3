...
int main( int argc, char * argv[] )
{
	...
	if (!fileOut.open( IO_WriteOnly ) ) {
	    tqWarning( "tquic: Could not open output file '%s'", outputFile.data() );
	    return 1;
	}
    } else {
	...
	TQUic::embed( out, projectName, images );
	return 0;
    }
	...
	tqWarning( "tquic: Could not open file '%s'", fileName );
	return 1;
    }
	...
	tqWarning( TQString("tquic: Failed to parse %s: ") + errMsg + TQString (" in line %d"), fileName, errLine );
	return 1;
    }

    TQDomElement e = doc.firstChild().toElement();
    if ( e.hasAttribute("version") && e.attribute("version").toDouble() > 3.3 ) {
	tqWarning( TQString("tquic: File generated with too recent version of TQt Designer (%s vs. %s)"),
    ...
    TQUic( fileName, outputFile, out, doc, !impl, subcl, trmacro, className, nofwd );
	...
	tqWarning( "tquic: Error writing to file" );
	if ( !outputFile.isEmpty() )
	    remove( outputFile );
    }
    return 0;
