...
*/
void TQUic::createFormDecl( const TQDomElement &e )
{
    TQDomElement n;
    TQDomNodeList nl;
    int i;
    ...
    */
	...
	for ( i = 1; i < (int) nl.length(); i++ ) { // begin at 1, 0 is the toplevel widget
	    ...
	    if ( s == "TQDataTable" ) { // other convenience classes which are used in TQDataTable signals, and thus should be forward-declared by tquic for us
...
void TQUic::writeFunctionsDecl( const TQStringList &fuLst, const TQStringList &typLst, const TQStringList &specLst )
{
 ...
 */
void TQUic::createFormImpl( const TQDomElement &e )
{
    TQDomElement n;
    TQDomNodeList nl;
    int i;
    ...
    */
	...
	for ( n = e; !n.isNull(); n = n.nextSibling().toElement() ) {
		...
		for ( i = 0; i < (int) nl.length(); i++ ) {
			...
			// resulting 'length' to catch corrupt TQUIC files?
			int a = 0;
			int column = 0;
			bool inQuote = FALSE;
...
void TQUic::createFormImpl( const TQDomElement& e, const TQString& form, const TQString& connection, const TQString& table )
{
    if ( e.tagName() == "widget" &&
