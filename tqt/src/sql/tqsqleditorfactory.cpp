...
static TQCleanupHandler< TQSqlEditorFactory > tqsql_cleanup_editor_factory;
...
TQSqlEditorFactory * TQSqlEditorFactory::defaultFactory()
{
	...
	tqsql_cleanup_editor_factory.add( &defaultfactory );
    }
...
void TQSqlEditorFactory::installDefaultFactory( TQSqlEditorFactory * factory )
{
    ...
    if( defaultfactory != 0 ){
	tqsql_cleanup_editor_factory.remove( &defaultfactory );
